...
app.get("/movies", function (req, res) {
  ...
  if (req.query.title) {
    ...
    if (!matches) {
      ...
      res.status(400).json({
        message: "Invalid titleQuery supplied",
      });
    } else {
      knex
        .select("*")
        ...
        .then((data) => {
          if (data.length === 0) {
            res.status(404).json({
              message: "Your query string returned no results",
            });
          }
          return data;
        })
...
app.post("/movies", function (req, res) {
  ...
  if (newMovie.title && newMovie.runtime && newMovie.release_year && newMovie.director) {
    knex("movies")
      .insert([
        { title: newMovie.title, runtime: newMovie.runtime, release_year: newMovie.release_year, director: newMovie.director },
      ])
      .returning("*")
      .then((data) => res.status(200).json(data))
      .catch((err) =>
        res.status(400).json({
          message: "Invalid Input From insert",
        })
      );
  } else {
    res.status(400).json({
